...
<div class="main-admin-cont">
	<div class="contanier">
		...
		<?php
        		...
        		$offset = ($page - 1) * $limit;
        		$sql = "SELECT * FROM product LEFT JOIN brands ON product.product_brand = brands.id ORDER BY product.product_id ASC LIMIT {$offset}, {$limit}";
		...
		<?php
				...
				if (mysqli_num_rows($result1) > 0) {
					...
					for ($i=1; $i <= $total_pages; $i++) {
						...
						echo '<li class="'.$active.'"><a href="admin-product-list.php?page='.$i.'" class="btn">'.$i.'</a></li>';
					}
					echo '</ul>';
				}
		 ?>
